...
const menuItems = [
  ...
  { icon: BarChart2, label: 'التداول',     id: 'trading'   },
  { icon: Briefcase, label: 'محفظتي',     id: 'portfolio' },
  { icon: Newspaper, label: 'أخبار الشركة', id: 'news'    },
  { icon: Settings,  label: 'الإعدادات',  id: 'settings'  },
];

const Sidebar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const { activePage, setActivePage, user } = useStore();
      ...
      >
              ...
              >
                ...
                <item.icon
                  className={cn(
                    ...
                    active ? 'text-primary-600 dark:text-primary-400' : 'group-hover:text-primary-500'
                  )}
                />
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800">
          <button
            ...
            className="w-full bg-slate-50 dark:bg-slate-800/60 rounded-2xl p-3.5 flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            <img
              src={user?.avatar}
